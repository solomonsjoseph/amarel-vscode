...
**You hand the user one command at a time. You do NOT execute these commands
yourself.** The user copies each command into their terminal, runs it, and
reports back. After each phase you confirm success (or diagnose a failure)
before giving them the next command.

This is intentional — passphrases, passwords, and host-key fingerprints must
be typed and verified by the user directly. You cannot see what they type
into a TTY, and you should never try to.
...
Capture the username as `$AMAREL_USER` for use in later commands. Substitute
it inline before showing each command (don't make the user edit placeholders).
