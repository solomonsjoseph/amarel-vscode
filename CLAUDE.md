...
- The phase numbering is load-bearing — error messages, the README troubleshooting table, `skills/amarel-vscode-setup/SKILL.md`, `AGENTS.md`, and `GEMINI.md` all reference phases by number. The runbooks run 0–12 (5.5 = remote-platform probe, 5.5b = NATIVE-only legacy-residue cleanup, 5.5c = NATIVE-only `amarel.rutgers.edu` alias so stale VS Code Recents don't land on the old host, 11 = Source Control git.path, 12 = optional GitHub); the scripts run 0–10 plus 5.5 (remote-platform probe), 5.6 (NATIVE-only legacy-residue cleanup), and 9.5 (the git.path step the runbooks surface as Phase 11). Don't renumber existing phases; append at the end, or insert with `.5`/`.6`.
