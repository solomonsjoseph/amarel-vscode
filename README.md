...
under load it can run to a few minutes, and the connection gives up before the
editor does. The
...
| A connect is slow and it is *not* a first connect to that node | This is the other cost: the SLURM leg, booking a node and waiting for it to start. Read `~/.amarel-dev-logs/connect.log`. Every line carries an absolute timestamp and an elapsed `+N.NNNs` since the connect began, so the stage that took the time names itself: `warm lookup`, `settle`, `provision`, `post-provision lookup`, `handing off to nc`. A warm reconnect is one lookup and a handoff; anything else means a job had to be booked or repaired. |
